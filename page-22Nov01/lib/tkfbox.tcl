...
# This is a modified version of the tcl source file of the same
# name. Mainly I changed all the ttk widgets to tk widgets by
# replacing all incidents of "ttk::" with "". The source file was
# found in tk8.6.10/library. I also changed the relief of f2.menu to
# sunken. In addition, I found it necessary to modify the foregrounds
# of several widgets, see ::tk::dialog::file::Create.  The source was
# found in tk8.6.10/library. Rozen
...
proc ::tk::dialog::file::Create {w class} {
    ...
    pack [frame $w.contents] -expand 1 -fill both
    #set w $w.contents
    
    # f1: the frame with the directory option menu
    #
    set f1 [frame $w.contents.f1]
    bind [::tk::AmpWidget label $f1.lab -text [mc "&Directory:"] \
              -foreground $::vTcl(pr,fgcolor)] \
        <<AltUnderlined>> [list focus $f1.menu]   ;# NEEDS WORK dark
    ...
    ;# NEEDS WORK dark
    menubutton $f1.menu -menu $data(dirMenu) -direction flush \
        -textvariable [format %s(selectPath) ::tk::dialog::file::$dataName]
    menu $data(dirMenu) -tearoff 0
    # Added the following statement so that in dark mode I whole menu
    # is readable.  Rozen.
    $data(dirMenu) configure \
    ...
    $f1.menu configure -relief sunken;# Rozen added relief option.
    $f1.menu configure -foreground $::vTcl(pr,fgcolor) ;# NEEDS WORK dark
    ...
    #
    set f2 [frame $w.contents.f2]
    bind [::tk::AmpWidget label $f2.lab -text $fNameCaption -anchor e\
              -foreground $::vTcl(pr,fgcolor)] \
        <<AltUnderlined>> [list focus $f2.ent] ;# NEEDS WORK dark
    # -pady 0
    # set data(ent) [entry $f2.ent];# NEEDS WORK dark
    set data(ent) [entry $f2.ent \
                       -foreground black -background white] ;# NEEDS WORK dark
    ...
    set data(typeMenuLab) [::tk::AmpWidget label $f2.lab2 \
               -text $fTypeCaption -anchor e -foreground $::vTcl(pr,fgcolor)]
    # -pady [$f2.lab cget -pady]
    ;# NEEDS WORK dark
    set data(typeMenuBtn) [menubutton $f2.menu \
          -foreground $::vTcl(pr,fgcolor) \
          -menu $f2.menu.m -relief sunken] ;# Rozen added relief option.
        # -indicatoron 1
    set data(typeMenu) [menu $data(typeMenuBtn).m -tearoff 0]
    # Added the following statement so that in dark mode I whole menu
    # is readable.  Rozen.
    $data(typeMenuBtn).m configure \
        -foreground $::vTcl(pr,fgcolor)  ;# NEEDS WORK dark
...
proc ::tk::dialog::file::Done {w {selectFilePath ""}} {
    ...
    set Priv(selectPath) $data(selectPath)
        if {($data(type) eq "save") && $data(-confirmoverwrite) && \
