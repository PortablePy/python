...
proc vTcl:set_command {target {option -command} \
    ...
    set r [vTcl:get_command "$opt for $alias" $cmd $base]
    if {$r == -1} { return }
    ...
    set vTcl(w,opt,-command) $cmd  ;# Rozen 9/11/14
    set $variable $cmd
...
proc vTcl:get_command {title initial base} {
    ...
    text $base.f.text \
        -insertborderwidth 3 \
        -background $vTcl(area_bg) -borderwidth 0 -height 3 -wrap none \
        -relief flat -width 20 -foreground black -cursor {} ;# NEEDS WORK dark
