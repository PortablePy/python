...
proc vTclWindow.vTcl.about {args} {
    ...
    set docs_dir [file join $vTcl(VTCL_HOME) "docs"]
    set about_text \
"
...
Copyright (C) Donald Rozenberg 2001-2021

Licensed under GNU General Public License
version 2


Documentation can be found\nin the \"$docs_dir\" subdirectory.

"
    dict set dicpdr "<page_dir>" [string map {/ \\} $vTcl(VTCL_HOME)]
    set about_text [string map $dicpdr $about_text]
...
proc vTcl:open_help_in_browser { } {
    ...
    if {$tcl_platform(platform) == "windows"} {
        ...
        set browser [exec xdg-settings get default-web-browser]
        if {[string first firefox $browser] > -1} {
            set command [list firefox --new-window  $help_file]
        } elseif {[string first "google-chrome" $browser] > -1} {
            set command [list google-chrome --new-window  $help_file]
        } elseif {[string first "opera" $browser] > -1} {
            set command [list opera --new-window file:///$help_file]
        } elseif {[string first "vivaldi" $browser] > -1} {
            set command [list vivaldi --new-window file:///$help_file]
        } elseif {[string first "brave" $browser] > -1} {
            set command [list brave-browser --new-window file:///$help_file]
    } else {

            set command [list xdg-open $help_file]
        }
