...
# name. All I did was to change all the ttk widgets to tk widgets by
# replacing all incidents of "ttk::" with "". In addition, I found it
# necessary to modify the foregrounds of several widgets. The source
# file was found in tk8.6.10/library. See ;# NEEDS WORK dark comments
# below. Rozen
...
proc ::tk::MessageBox {args} {
    ...
    if {([winfo depth $w] < 4) || $tk_strictMotif} {
        # label has no -bitmap option
        label $w.bitmap -bitmap $data(-icon) -background $bg ;# NEEDS WORK dark
    } else {
        switch $data(-icon) {
                error {
                    label $w.bitmap -image ::tk::icons::error
                }
                info {
                    label $w.bitmap -image ::tk::icons::information
                }
                question {
                    label $w.bitmap -image ::tk::icons::question
                }
                default {
                    label $w.bitmap -image ::tk::icons::warning
                }
        }
    }
    }
    ...
    if {![llength $opts]} {
        ...
        # eval [list tk::AmpWidget button $w.$name] $opts \   ;# NEEDS WORK dark
        #     [list -command [list set tk::Priv(button) $name]]
    eval [list tk::AmpWidget button $w.$name -foreground $fg] $opts \
        [list -command [list set tk::Priv(button) $name]]
