...
# Time-stamp: <2022-04-20 12:59:16 rozen>%
# $Id: $
#-------------------------------------------------------------------------------
# Inspired by:
...
namespace eval ::colorDlg {
    ...
    #package require Tk 8.6     # 2/10/15 Not needed; 8.6 required for Page.
...
proc ::colorDlg::colorDlg {args} {
    ...
    eval "::colorDlg::lng_en"    ;# Forces the color chooser to be in English.
    ...
    set ::colorDlg::options(-background) $::vTcl(tk_default_bgcolor) ;# Rozen
    set ::colorDlg::options(-foreground) $::vTcl(tk_default_fgcolor) ;# Rozen
  ...
  set ::colorDlg::widgets(chs) $w.c.t.gradient
    ...
    # # Added bindings to allow paste of color. Rozen 7/29/19
    # bind $w.c.t.ecolor <Button-2> {
    #     if {![catch {set clip [selection get -selection CLIPBOARD]}]} {
    #         set ::colorDlg::currents(color) #$clip
    #         ::colorDlg::validateColor
    #     }
    # }
    # bind $w.c.t.ecolor <ButtonRelease-2> {
    # }
...
proc ::colorDlg::complement {color} {
    ...
    lassign $rgb r g b
    if {$r == $g && $r == $b} {
        # Color is a monochrome.
        if {[::colorDlg::dark_color $color]} {
            return  #d9d9d9  ;# Gray85
        } else {
            return  gray40   ;# #666666
        }
    }
...
proc ::colorDlg::dark_color {color} {
    ...
    # Alternative calculation used in page/color_chart/color_chart.py
    set h \
...
proc ::colorDlg::monochrome {color} {
    set rgb [::colorDlg::colorToRgb $color]
    lassign $rgb r g b
    if {$r == $g && $r == $b} {
        # Color is a monochrome.
...
proc ::colorDlg::analogous_colors {color} {
    ...
    if {$r == $g && $r == $b} {
        # Color is a monochrome.
        if {[::colorDlg::dark_color $color]} {
            return [list #d9d9d9 #c3c3c3]
        } else {
            return [list #c3c3c3 #d9d9d9]
        }
    }
    set hsv [rgbToHsv $r $g $b]
    lassign $hsv h s v
...
set ::vTcl(tk_default_bgcolor) #d9d9d9    ;# NEEDS WORK
set ::vTcl(tk_default_fgcolor) #000000
