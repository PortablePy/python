...
namespace eval ::menu_edit {
    ...
    proc {::menu_edit::fill_menu_list} {top m} {
        ...
        # Here we put in the first item <Menu> at the top of
    ...
    proc {::menu_edit::show_menu} {top index} {
        ...
        set i [lindex $reference end]
        ###########################################################
        # Beginning of coloring the listbox on the right hand side.
...
proc configCmd {top option variable} {
    ...
    if {$i < 0} {  # Rozen to allow setting of <Menu> options.
        ...
        ;# NEEDS WORK  Removed this 7/24/19
        if {$option == "-command"} {
            set v "#$v"
        }
        set listboxitems [set ::${top}::listboxitems]
        ...
        while {$l <= $i} {
            ...
            if {$menu == $start_menu} {
                if {[regexp {<Menu>} $selection_v] == 0} {
                    incr j
                }
            }
            incr l
        }
        if {[winfo parent $start_menu] ne "."} {
            set k [expr {$j - 1}]
        } else {
            set k $j
        }
        $m entryconfigure $k $option $v
...
proc vTclWindow.vTclMenuEdit {base menu} {
    ...
    button $base.fra21.but22 \
        ...
        -image $image \
        -command "::menu_edit::ask_delete_menu $base"
    # vTcl:toolbar_label  $base.fra21.but22 \
    #     -image $image \
    #     -command "::menu_edit::ask_delete_menu $base"
    # Up button.
    ...
    ;# NEEDS WORK dark changed background blow
    # listbox $base.cpd24.01.cpd25.01 -background #d9d9d9 -foreground black
    listbox $base.cpd24.01.cpd25.01 -background $vTcl(area_bg) -foreground black
