...
proc vTcl:open {{file ""} {title "Open Project"}} {
    ...
    # 'Plum' to alter the user that this is source of widget for
...
# set qz [place configure .top44.but45]
# dmsg -s
# dpr qz
}
...
proc vTcl:save2 {file {save_type {"reg"}}} {
    ...
    #     unset vTcl(set_sash)
    # }
    set vTcl(validate_functions) [list]
    if {$file == ""} {
        return -1
    }
...
# NEEDS WORK - Replace following block of code.
    ...
    if {$save_type eq "reg"} {
        ...
        for {set n $m} {$n>0} {incr n -1} {
            ...
            catch {[file copy -force $file.bak$n $file.bak$n_plus_1]}
        }
...
proc vTcl:quit {} {
    ...
    set vTcl(quit) 1
    #set vTcl(change) 1
    set closing_geometry [wm geometry $vTcl(real_top)]
    if {$closing_geometry ne $vTcl(top_geometry)} {
        set vTcl(change) 1
    }
    set vTcl(autosave_skip) 1
...
# dmsg -s saving    
# set geometry_vTcl [wm geometry .vTcl]
# set geometry_toolbar [wm geometry .vTcl.toolbar]
# set geometry_tree [wm geometry .vTcl.tree]
# dpr geometry_vTcl
# dpr geometry_toolbar
# dpr geometry_tree
# set winfo_geom_tree [winfo geometry .vTcl.tree]
# dpr winfo_geom_tree
# dskip    
    # Create list of open startup windows. vTcl(startup_windows) set
    # in global.tcl.
