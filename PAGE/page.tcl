...
# Copyright (C) 2020-2021 Donald Rozenberg
#
...
proc vTcl:load_widgets {} {
    ...
    # set toload [vTcl::project::getLibrariesToLoad] removed 3/6/19
    ## Make sure lib_core loads before all other widget libraries.
    #::vTcl::lremove toload *lib_core.tcl   removed 2/6/19
...
proc vTcl:setup {} {
    ...
    if {$vTcl(select_profile)} {
        ...
        set file [tk_getOpenFile -defaultextension "" -title $title \
                      -initialdir $home_dir]
        if {$file == ""} {
    ...
    set LIBS            "globals.tcl about.tcl propmgr.tcl balloon.tcl
             bgerror.tcl bind.tcl command.tcl color.tcl tkcon.tcl
             ...
             convert.tcl multiselect.tcl busy_cursor.tcl undo.tcl"

    # Removed from above  tclet.tcl attrbar.tcl proc.tcl var.tcl compounds.tcl
    # Removed from above  new.tcl help.tcl ttd/ttd.tcl
...
proc vTcl:setup_gui {} {
    ...
    foreach i $vTcl(gui,showlist) {
        ...
        if {$i == ".vTcl.help"} continue  ;# Rozen  Don't show help.
...
proc vTcl:define_rc_menus {} {
    ...
    # Spread Horizontal
    $vTcl(gui,rc_multi_menu) add separator
    $vTcl(gui,rc_multi_menu) add comm -label "Spread Horizontal" -comm {
        vTcl:spread_horizontal
    }
    # Spread Vertical
    $vTcl(gui,rc_multi_menu) add comm -label "Spread Vertical" -comm {
        vTcl:spread_vertical
    }
    # Center Horizontal
    $vTcl(gui,rc_multi_menu) add separator
    $vTcl(gui,rc_multi_menu) add comm -label "Center Horizontal" -comm {
        vTcl:center_horizontal
    }
    # Center Vertical
    $vTcl(gui,rc_multi_menu) add comm -label "Center Vertical" -comm {
...
#}
    ...
    catch {package require Img}    ; #for better image support - Rozen
    set vTcl(select_profile) 0
    ...
    if {$argv == ""} {
        ...
        set vTcl(profile) ".pagerc"  ;# Default value in case -p not specified.
        set vTcl(select_profile) 0
        set vTcl(skip_profile) 0
        switch $first_token {
            -p {
            ...
            -s {
                # Use get file dialog to chose the profile rc.
    ...
    proc create_blank_toplevel {} {
        ...
        destroy .x
        vTcl:new_widget 0 "Toplevel" bbb
        set vTcl(top_geometry) [wm geometry $vTcl(tops)]
        set vTcl(real_top) [lindex $vTcl(tops) 0]
...
if {$file != ""} {
    ...
    # # Create the namespace so that I can store the widget info
    # # there. In particular the design mode.
    # namespace eval ::widgets::${vTcl(real_top)} {
    #     variable mode
    # }
    # if {$vTcl(pr,relative_placement) == 1} {
    ...
    #     } else {
    #         # Project name given but <project>.tcl does not exist. So
    #         # we want to open a clean toplevel widget.
    #         if {[winfo exists .x]} {destroy .x}
    ...
    #     # set vTcl(proj_dir) [string cat "./" [string trim ${vTcl(proj_dir)}]]
    #     set vTcl(proj_dir) [string cat [string trim ${vTcl(proj_dir)}]]
    #  } else {
    #     # No project name given.
    #     set vTcl(project_specified) 0
    #     set vTcl(proj_dir) "."           ;# 5/23/19
    # }
    # set vTcl(initialdir) $vTcl(proj_dir)
    # if [info exists vTcl(pr,autoloadcomp)] {
    #     if {$vTcl(pr,autoloadcomp)} {
    #         set loaded [vTcl:load_compounds $vTcl(pr,autoloadcompfile)]
    #         if {$loaded == ""} {
    #             ## if user cancels, we unselect the "autoload c/home/rozen/page-6.0.1/pageompounds" option
...
#trace add variable vTcl(undo,*,list) \
    write "tracer vTcl(undo,*,list)"

#trace add variable vTcl(actual_gui_font_dft_desc) \
    write "tracer vTcl(actual_gui_font_dft_desc)"sunken

#trace add variable vTcl(tops) \
    write "tracer vTcl(tops)"

#namespace eval ::widgets::.top44 { }
#trace add variable ::widgets::.top44::dflt,origin \
    write "tracer ::widgets::.top44::dflt,origin"
