...
proc vTcl:show_propmgr {} {
    global vTcl
    vTclWindow.vTcl.ae
    # wm geometry .vTcl.ae $vTcl(default,.vTcl.ae)
...
proc vTclWindow.vTcl.ae {args} {
    ...
    bind $w.ea <KeyRelease-Return> "
        ...
        set vTcl(change) 1; vTcl:show_wtree" ;# Line added 5/27/16 & 4/4/17. Rozen
    bind $w.ea <FocusOut> "
        ::vTcl::properties::setAlias \$::vTcl(w,widget) ::vTcl(w,alias) $w.ea
        set vTcl(change) 1
    ...
    # if { $vTcl(w,widget) != "" } {  ;# NEEDS WORK for AE scrolling problem.
    #     vTcl:prop:update_attr
    # }
...
proc vTcl:prop:config_cmd {target option variable value args} {
    ...
    if {$value == ""} {
        ...
        # switch -exact $option {
        #     -textvariable {
        #         $target configure -textvariable ::vTcl::$value
    ...
    if {[winfo exists $top] } {
        ...
        set mgr $vTcl(w,manager)
            ...
            if {[lsearch -exact $vTcl(m,$mgr,extlist) "title"] > 0} {
                ...
                # section to attribute section.
                set j "title"
                set mgr $vTcl(w,manager)
                set variable "vTcl(w,$mgr,$j)"
                set cmd [lindex $vTcl(m,$mgr,$j) 4]
                if {$cmd == ""} {
    ...
    #catch {
        ...
        if {[vTcl:is_geom_fluid $vTcl(w,widget)]} {
            ...
            foreach i "$vTcl(m,$mgr,extlist) $vTcl(m,$mgr,list)" {
                if {$i == "title"} continue ;# Rozen moved title from
                #                              # geom section.
                # if {$i eq "dflt,origin" && $vTcl(w,class) ne "Toplevel"} {
                #     continue
                # }
                # if {$i eq "-height" && $vTcl(w,class) in \
                #         {TProgressbar TCheckbutton TRadiobutton \
                #              TSizegrip}} { continue }
                # if {$i eq "-width" && $vTcl(w,class) in \
                #         { TSizegrip}} { continue }
#                 if {$vTcl(w,class) eq "TSeparator"} {
# dpr w
#                     set orient [$w cget -orient]
# dpr orient i
#                     if {$orient == "vertical" && $i eq "-width"} {
# dmsg skip -width
#                         continue
#                     } elseif {$orient == "horizontal" && $i eq "-height"} {
# dmsg skip -height
#                         continue
#                     }
#                 }
...
proc vTcl:prop:new_attr {top option variable config_cmd \
    ...
    switch $type {
        george {       ;# trial 'type'
            # This branch originally done in default branch, moved
            # here simply to increment vTcl(change).
            frame $base
            entry ${base}.l -relief sunken  \
...
#     set gpc [vTcl:get_class $grand_parent]
# dpr gpc  opt  w
#     if {$gpc eq "TNotebook"} {
#         if {$opt eq "-x" || $opt eq "-y"} {
