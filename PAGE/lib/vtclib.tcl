...
proc vTcl:fixup_alias {alias} {
    ...
    global vTcl
#dtrace
#    set pieces [split $alias "_"]
#    set len [llength $pieces]
#    if {$len > 1} {
#        set last_piece [lrange $pieces end end]
#        if {[string is integer $last_piece] == 1 } {
#dmsg spot A1
#            # last piece is an integer so just replace it with one
#            # that would be distinguishing.
#            set new_pieces [lrange $pieces 0 end-1]
#            if {$last_piece <= $vTcl(alias_cnt)} {
#                # to make sure we don't reuse a value.
#                set vTcl(alias_cnt) [expr $last_piece + 1]
#            }
#            lappend new_pieces _$vTcl(alias_cnt) ;# add new value
#            set alias [join $new_pieces ""] ;# jam them together.
#            incr vTcl(alias_cnt)
#        } else {
#            set alias ${alias}_$vTcl(alias_cnt)
#            incr vTcl(alias_cnt)
#        }
#    } else {
#        set alias ${alias}_$vTcl(alias_cnt)
...
proc vTcl:compile_alias {top} {
    ...
    set existing [] ;# In case there no existing aliases.
    foreach ii $tree {
            ...
            if {[lsearch -exact $existing $alias] != -1} {
                if {[info exists widget($toplevel,$alias)]
                    ...
                    if {$target_class eq "Frame" && \
                            $parent_class eq "TNotebook"} {
                        set alias [vTcl:fixup_alias $alias]
                    } elseif {$target_class eq "Frame" && \
                                  $parent_class eq "PNotebook"} {
                        set alias [vTcl:fixup_alias $alias]
                    }
                }
            }
